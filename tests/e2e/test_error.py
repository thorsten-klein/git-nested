...
def assert_argparse_error(cp, error_msg, returncode=2):
    ...
    assert "usage: git nested" in cp.output
    assert error_msg in cp.output


def assert_error(cp, error_msg, returncode=2):
    """Assert the command failed and said `error_msg`, on whichever stream it chose."""
    assert cp.returncode == returncode
    assert error_msg in cp.output
...
def test_error_unknown_command(foo_bar_cloned):
    ...
    assert (text in cp.output) or (text.replace("'", "") in cp.output)
...
def test_error_extra_arguments_for_clone(foo_bar_cloned):
    ...
    assert_error(cp, "git nested: error: unrecognized arguments: baz quux")
