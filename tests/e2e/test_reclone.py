...
def test_reclone(foo_bar_cloned):
    ...
    assert cp.output.strip() == f"Nested repository '{env.upstream}/bar' (master) cloned into 'bar'."
    ...
    assert cp.output.strip() == "Nested repository 'bar' is up to date with upstream branch 'master'."
