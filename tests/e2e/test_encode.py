...
def test_encode(foo_bar_cloned, dirname, expected_dirname, sanitized_ref):
    ...
    assert result.output.strip() == f"{expected_dirname}: cloned from {env.upstream}/bar (master)"
    ...
    assert result.output.strip() == f"{expected_dirname}: pulled from {env.upstream}/bar (master)"
    assert (env.workspace / 'bar' / 'Bar2').exists()
