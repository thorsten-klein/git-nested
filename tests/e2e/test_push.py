...
def test_nested_push(foo_bar_cloned_and_nested):
    ...
    assert result.output.strip() == "git-nested: There are new changes upstream (master), you need to pull first."
...
def test_push_pull_multiple_nested(foo_bar_cloned_and_nested):
    ...
    assert result.output.strip() == f"Nested repository 'bar' pushed to '{bar_dir}' (master)."
...
def test_push_pull_feature_branch(foo_bar_cloned_and_nested):
    ...
    assert result.output.strip() == f"Nested repository 'bar' pulled from '{bar_upstream}' ({env.defaultbranch})."
    ...
    assert result.output.strip() == f"Nested repository 'bar' pushed to '{bar_upstream}' ({env.defaultbranch})."
...
def test_push_after_init(env):
    ...
    assert result.output.strip() == "Nested repository 'doc' pushed to '../upstream' (init-master)."
...
def test_push_new_branch(foo_bar_cloned_and_nested):
    ...
    assert result.output.strip() == f"Nested repository 'bar' pushed to '{env.upstream}/bar' (newbar)."

    # Do the nested push to another branch again
    result = cmd_git_nested('push bar --branch newbar', cwd=env.workspace / 'foo')
    assert result.output.strip() == "Nested repository 'bar' has no new commits to push."
...
def test_push_no_changes(foo_bar_cloned_and_nested):
    ...
    assert cp.output.strip() == "Nested repository 'bar' has no new commits to push."
...
def test_push_squash_with_explicit_branch_errors(foo_bar_cloned_and_nested):
    ...
    assert result.returncode == 1
    assert result.output.strip() == "git-nested: Squash option (-s) can't be used with branch parameter"
