"""Unit tests for git-nested business-logic branches, using FakeGit instead of real git repos"""
...
def test_extract_gitrepo_commit_returns_none_when_missing():
    ...
    assert history._extract_gitrepo_commit(git, 'deadbeef', Path('sub')) is None
...
def test_extract_gitrepo_commit_returns_stripped_commit():
    git = FakeGit().respond('cat-file', '-p', stdout='nested:\n  commit: deadbeef\n')
    assert history._extract_gitrepo_commit(git, 'somecommit', Path('sub')) == 'deadbeef'
...
def test_push_fetch_missing_upstream_true_when_ref_not_found():
    assert push._push_fetch_missing_upstream("fatal: couldn't find remote ref refs/heads/foo\n") is True
...
def test_format_refs_empty_when_no_matching_refs():
    ...
    assert status.format_refs(git, 'sub') == ""
...
def test_format_refs_formats_known_ref_types():
    ...
    output = status.format_refs(git, 'sub')
...
def test_status_header_no_nested_repositories():
    header, done = status._status_header(Flags(quiet=False), count=0)
    assert header == "No nested repositories.\n"
    assert done is True


def test_status_header_quiet_is_always_empty():
    header, done = status._status_header(Flags(quiet=True), count=3)
    assert header == ""
    assert done is False


def test_status_header_singular_vs_plural():
    header, _ = status._status_header(Flags(quiet=False), count=1)
    assert header == "1 nested repository:\n"
    header, _ = status._status_header(Flags(quiet=False), count=2)
...
def test_status_for_subdir_not_a_nested_repository(tmp_path, monkeypatch):
    ...
    lines, entries = status._status_for_subdir(git=git, flags=Flags(), git_tmp=tmp_path, subdir=subdir)
    assert entries == []
...
def test_get_status_no_nested_repositories():
    ...
    output, entries = status.get_status(git, Flags(quiet=False), git_tmp=Path('/tmp/gt'))
...
def test_status_for_subdir_fetches_when_flag_set(tmp_path, monkeypatch):
    ...
    lines, entries = status._status_for_subdir(
...
def test_status_identity_lines_branch_and_remote_present():
    ...
    output = status._status_identity_lines(git, 'sub', config, upstream_head='abc1234')
    joined = ''.join(output)
...
def test_status_detail_lines_appends_refs_when_verbose():
    ...
    config = NestedConfig(remote='https://example.com/x.git', branch='main')
    lines = status._status_detail_lines(
...
def test_create_nested_branch_reuses_existing_branch(tmp_path):
    ...
    config = NestedConfig(remote='https://example.com/x.git', branch='main')
    worktree = content.create_nested_branch(
        git=git,
        flags=Flags(),
        config=config,
        branch='nested/sub',
...
def test_process_chain_commit_skips_commit_without_gitrepo_data():
    ...
    history._process_chain_commit(git, Flags(), config, Path('sub'), 'sub', 'pull', 'deadbeef', state)
